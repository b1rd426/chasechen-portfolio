/* Adapted from ThreeUI Community (MIT), copyright 2026 Meng To.
 * See public/garden/THREEUI-LICENSE and lib/garden/README.md.
 * Local changes: scroll camera, pause support, bounded DPR, responsive sizing,
 * display-space grain, coherent wind, leaf aerodynamics and layered atmosphere. */
/* Upstream generated world; adapted for this portfolio. */
...
/* A shared, slowly changing breeze gives independent objects a common cause.
   Leaves add local curl to this field; foliage and rain receive the same gust. */
const windNoise = noise2D(20260919);
const wind = { x: .7, z: .15, gust: .5, uniform: { value: new THREE.Vector3(.7, .15, .5) } };
const weatherRandom = mulberry32(619);
function updateWind() {
  const t = clock * .16;
  const broad = windNoise(t, 4.27) * .5 + .5;
  wind.gust = .25 + Math.pow(broad, 2) * 2.1;
  const angle = windNoise(t * .48, 19.3) * 1.25;
  wind.x = Math.cos(angle) * wind.gust;
  wind.z = Math.sin(angle) * wind.gust;
  wind.uniform.value.set(wind.x, wind.z, wind.gust);
}
...
function texShoji() {
  ...
  x.lineWidth = 24; x.strokeStyle = 'rgba(8,6,5,.95)';
...
function texMoon() {
  ...
  x.globalCompositeOperation = 'overlay'; x.globalAlpha = .16;
  ...
  seas.forEach(([u, v, rad, dk]) => {
    ...
    for (let i = 0; i < 22; i++) {
      ...
      bg.addColorStop(0, 'rgba(0,0,0,' + (dk * .06).toFixed(3) + ')');
  ...
  x.save(); x.filter = 'blur(12px)'; x.globalAlpha = .48;
...
const DPR_CAP      = Math.min(qn('dpr', LOW ? 1 : 1.25), 1.5);
const PIXEL_BUDGET = LOW ? 700000 : 1800000;
const pixelRatio = () => Math.min(devicePixelRatio || 1, DPR_CAP, Math.sqrt(PIXEL_BUDGET / (vpW() * vpH()))) * PERF.scale;
...
const PERF = { scale: 1, pending: 1, acc: 0, n: 0, resizedAt: -10, locked: qs('adapt', '1') === '0' };
...
function buildTemple() {
  ...
  // A gently lit paper surface keeps the windows from reading as solid blocks.
  const paperCanvas = cvs(128, 256), paperContext = paperCanvas.getContext('2d');
  const paperLight = paperContext.createRadialGradient(64, 116, 12, 64, 150, 170);
  paperLight.addColorStop(0, '#fff1cf');
  paperLight.addColorStop(.55, '#c7b79a');
  paperLight.addColorStop(1, '#655947');
  paperContext.fillStyle = paperLight; paperContext.fillRect(0, 0, 128, 256);
  const paper = new THREE.MeshBasicMaterial({ map: tx(paperCanvas), color: hdr(1.0, .57, .29), fog: true, toneMapped: false });
...
const MOON = { x: 20.4, y: 30.5, z: -72, r: 7.2 };
function buildMoon() {
  const disc = new THREE.Mesh(new THREE.PlaneGeometry(MOON.r * 2, MOON.r * 2),
    ...
    new THREE.MeshBasicMaterial({ map: tx(texMoon()), color: hdr(2.4, .92, .68),
  ...
  // Soft, elongated cloud banks pass in front of the moon, behind the roof.
  // A small shared texture supplies irregular edges without a full-screen pass.
  const cloudCanvas = cvs(256, 96), context = cloudCanvas.getContext('2d');
  const pixels = context.createImageData(256, 96), cloudNoise = noise2D(317);
  for (let y = 0; y < 96; y++) for (let x = 0; x < 256; x++) {
    const nx = x / 256, ny = y / 96;
    const shape = Math.pow(Math.max(0, Math.sin(nx * Math.PI) * Math.sin(ny * Math.PI)), .8);
    const density = sat(.5 + fbm(cloudNoise, nx * 5.3, ny * 2.6, 4, 2, .5) * 1.8);
    const offset = (y * 256 + x) * 4;
    pixels.data[offset] = 15; pixels.data[offset + 1] = 23; pixels.data[offset + 2] = 27;
    pixels.data[offset + 3] = shape * density * 235;
  }
  context.putImageData(pixels, 0, 0);
  const cloudTexture = tx(cloudCanvas);
  WORLD.moonClouds = [];
  for (let i = 0; i < 3; i++) {
    const cloud = new THREE.Mesh(new THREE.PlaneGeometry(37 + i * 5, 6 + i * 2),
      new THREE.MeshBasicMaterial({ map: cloudTexture, transparent: true, depthWrite: false, fog: false, opacity: .55 }));
    cloud.position.set(MOON.x - 7 + i * 3, MOON.y - 6 + i * 6, MOON.z + 3 + i);
    cloud.userData = { phase: i * 2.3, offset: -7 + i * 3, y0: cloud.position.y };
    cloud.renderOrder = 2;
    scene.add(cloud); WORLD.moonClouds.push(cloud);
  }
}
...
function buildMaple(seed, x, z, scale) {
  ...
  leafMat.onBeforeCompile = sh => {
    ...
    sh.uniforms.uWind = wind.uniform;
    sh.vertexShader = 'uniform float uT; uniform vec3 uWind;\n' + sh.vertexShader.replace('#include <begin_vertex>',
      '#include <begin_vertex>\n' +
      'vec3 wp = (instanceMatrix * vec4(0.0,0.0,0.0,1.0)).xyz;\n' +
      'float ph = wp.x*1.7 + wp.z*1.3 + wp.y*.7;\n' +
      'float flutter = sin(uT*2.15 + ph) * sin(uT*.47 + ph*.3);\n' +
      'transformed.x += (uWind.x*.10 + flutter*.07) * (0.5+wp.y*.12);\n' +
      'transformed.z += (uWind.y*.09 + cos(uT*1.3+ph)*.045) * (0.5+wp.y*.12);\n');
  };
  return g;
}
...
function cutoutMaterial(canvasEl, sway) {
  ...
  mat.onBeforeCompile = sh => {
    sh.uniforms.uT = WORLD.uT;
    sh.uniforms.uSway = { value: sway > .015 ? sway : 0 };
    sh.uniforms.uWind = wind.uniform;
    sh.vertexShader = 'uniform float uT;\nuniform float uSway;\nuniform vec3 uWind;\n' + sh.vertexShader.replace(
      '#include <begin_vertex>',
      '#include <begin_vertex>\n' +
      'float h = uv.y;\n' +
      'float ph = position.x*.42 + position.z*.2;\n' +
      'float wave = sin(uT*1.1 + ph) + .38*sin(uT*2.3-ph*.7);\n' +
      'transformed.x += (uWind.x*1.8 + wave*(.5+uWind.z))*uSway*h*h;\n' +
      'transformed.y += cos(uT*.72 + ph)*uSway*.42*h*h*uWind.z;\n');
...
function buildAtmosphere() {
  ...
  for (let i = 0; i < (LOW ? 4 : 6); i++) {
    ...
    h.renderOrder = 4;
    h.userData = { sp: .12 + rnd() * .22, ph: rnd() * TAU, x0: h.position.x, y0: h.position.y, opacity: h.material.opacity };
  ...
  const emb = new THREE.Points(g, new THREE.ShaderMaterial({
    uniforms: { uT: WORLD.uT, uWind: wind.uniform, uTex: { value: tx(texGlow('rgba(255,190,140,1)', 'rgba(255,120,60,.35)')) },
      uSize: { value: vpH() * .5 } },
    transparent: true, blending: THREE.AdditiveBlending, depthWrite: false,
    vertexShader:
      'attribute float aSeed; uniform float uT; uniform vec3 uWind; uniform float uSize; varying float vA;\n' +
      'void main(){ vec3 p=position;\n' +
      ' p.y += sin(uT*(.21+aSeed*.12)+aSeed*31.0)*.8;\n' +
      ' p.x += sin(uT*.28+aSeed*22.0)*1.4 + uWind.x*.35;\n' +
      ' p.z += cos(uT*.19+aSeed*17.0)*1.1 + uWind.y*.35;\n' +
      ' vec4 mv = modelViewMatrix * vec4(p,1.0);\n' +
      ' vA = (.22+.78*pow(.5+.5*sin(uT*(.6+aSeed*.8)+aSeed*49.0),3.0)) * (1.0-smoothstep(7.0,11.5,p.y)) * smoothstep(0.0,1.4,p.y);\n' +
  ...
  if (!LOW) {
    ...
    const rain = new THREE.LineSegments(rg, new THREE.ShaderMaterial({
      uniforms: { uT: WORLD.uT, uWind: wind.uniform },
      transparent: true, blending: THREE.AdditiveBlending, depthWrite: false,
      vertexShader:
        'attribute float aTop; attribute float aSpeed; attribute float aLen; uniform float uT; uniform vec3 uWind; varying float vA;\n' +
        'void main(){ vec3 p=position;\n' +
        ' float y = mod(p.y - uT*aSpeed, 17.0);\n' +
        ' p.y = y + aTop*aLen; p.x += uWind.x*(17.0-p.y)*.12; p.z += uWind.y*(17.0-p.y)*.12;\n' +
...
function buildLeafFall() {
  ...
  const mat = new THREE.MeshStandardMaterial({
    ...
    color: 0x693b29, roughness: .84, metalness: 0,
    /* Muted copper and rust hold detail against the warm lantern light. */
    emissive: 0x67220c, emissiveIntensity: .48
  });
  ...
  for (let i = 0; i < N; i++) list.push({
    ...
    fall: .45 + rnd() * .85, vx: 0, vz: 0, vy: 0,
    lift: .3 + rnd() * .8,
    sway: .45 + rnd() * 1.5, swayPh: rnd() * TAU, swayAmp: .30 + rnd() * .95,
    spin: (rnd() - .5) * 2.6, roll: rnd() * TAU, rollSp: .5 + rnd() * 2.0,
    tilt: rnd() * TAU, s: .55 + rnd() * .9
  });
  const palette = [0xe5a474, 0xb95d40, 0xc88451, 0x974734];
  for (let i = 0; i < N; i++) inst.setColorAt(i, new THREE.Color(palette[i % palette.length]));
  scene.add(inst);
...
function updateLeaves(dt) {
  ...
  for (let i = 0; i < L.length; i++) {
    const l = L[i];
    // Curl of a smooth potential: neighbouring leaves drift together, then
    // separate through their own drag/lift. Velocity damping is time-based.
    const nx = l.x * .12 + clock * .09, nz = l.z * .12;
    const curlX = (windNoise(nx, nz + .08) - windNoise(nx, nz - .08)) / .16;
    const curlZ = -(windNoise(nx + .08, nz) - windNoise(nx - .08, nz)) / .16;
    const flutter = Math.sin(clock * l.sway + l.swayPh);
    l.vx = damp(l.vx, wind.x * 1.35 + curlX * .75, 1.8 / l.s, dt);
    l.vz = damp(l.vz, wind.z + curlZ * .65, 1.8 / l.s, dt);
    l.vy = damp(l.vy, -l.fall + flutter * .26 + (wind.gust - .65) * l.lift, 2.4, dt);
    l.x += l.vx * dt; l.z += l.vz * dt; l.y += l.vy * dt;
    l.roll += l.rollSp * (.55 + wind.gust * .4) * dt;
    l.tilt += (l.spin * .65 + curlX * .4) * dt;
    if (seed || l.y < cy - 10) {                /* back to the top of the band */
      l.y = seed ? cy - 10 + weatherRandom() * 26 : cy + 16;
      const a = weatherRandom() * TAU, r = Math.sqrt(weatherRandom()) * LEAF_SPREAD;
      l.x = fx + Math.cos(a) * r;
      l.z = fz + Math.sin(a) * r;
    }
...
function initPost() {
  ...
  POST.scene = new THREE.WebGLRenderTarget(w, h, Object.assign({}, O, { depthBuffer: true }));
  let lw = Math.max(2, w >> 2), lh = Math.max(2, h >> 2);
  const N = LOW ? 2 : 3;
  for (let i = 0; i < N; i++) {
  ...
  POST.comp = new THREE.ShaderMaterial({
    uniforms: {
      ...
      uT: { value: 0 }, uBloom: { value: .34 }, uCA: { value: 1 }, uGrain: { value: .012 },
...
/* ================================================= 8 · the camera rig */
const CAM = [
  { p: [ -1.8, 4.80, 20.0 ], t: [ -3.0, 8.00, -18.0 ], fov: 39 },  /* 0 hero        */
...
const RIG = { prog: 0, smooth: 0, mx: 0, my: 0, tmx: 0, tmy: 0, intro: 0, focus: -1, focusAmt: 0, scrolledAt: -10 };
...
function applyCamera() {
  ...
  // In portrait, lift the sight line to place the hall below the introduction.
  // The adjustment fades as the visitor leaves the opening chapter.
  _t.y += aspectFix() * 9.0 * (1 - smooth(0, 1, RIG.smooth));

  /* the opening dolly: a long lens easing in from further back */
  const io = 1 - RIG.intro;
  _p.z += io * 1.4; _p.y += io * .18; fov += io * .8;

  /* parallax — a hand-held drift, never enough to break the frame */
  const par = 1 - smooth(0, 1.6, RIG.smooth) * .55;
  const driftX = windNoise(clock * .055, 7.2) * .32;
  const driftY = windNoise(clock * .043, 31.8) * .13;
  _p.x += (RIG.mx * .62 + driftX) * par;
  _p.y += (RIG.my * .34 + driftY) * par;
...
/* Host lifecycle and local composition/weather adaptations. */
function resize(measureViewport = true) {
  if (measureViewport) {
  ...
  const changed = renderer.domElement.width !== Math.floor(w * ratio) || renderer.domElement.height !== Math.floor(h * ratio);
  ...
  if (changed) {
    ...
    if (WANT_POST && POST.scene) {
      ...
      let lw = Math.max(2, pw >> 2), lh = Math.max(2, ph >> 2);
      POST.levels.forEach(L => { L.a.setSize(lw, lh); L.b.setSize(lw, lh); L.w = lw; L.h = lh; lw = Math.max(2, lw >> 1); lh = Math.max(2, lh >> 1); });
    }
  }
  // The baked light-space shadow does not depend on the canvas dimensions.
...
function updateWorld(dt) {
  ...
  const warmth = windNoise(clock * .22, 42.7);
  const f = RIG.focusAmt;
  if (WORLD.hallHalo) WORLD.hallHalo.material.opacity = .30 + f * .14 + warmth * .028;
  if (WORLD.hallLight) WORLD.hallLight.intensity = 3.4 * (1 + f * .30) * (1 + warmth * .035);
  /* the moon only breathes — the haze in front of it is what actually moves */
  if (WORLD.moonHalo) WORLD.moonHalo.material.opacity = .44 + f * .10 + warmth * .018;
  if (WORLD.moonClouds) WORLD.moonClouds.forEach((cloud) => {
    const u = cloud.userData;
    cloud.position.x = WORLD.moon.position.x + u.offset + Math.sin(clock * .043 + u.phase) * 8;
    cloud.position.y = u.y0 + windNoise(clock * .07, u.phase + 9) * .65;
  });
  if (WORLD.lanternLights) WORLD.lanternLights.forEach((l, i) => {
    const flicker = windNoise(clock * 1.7, i * 3.7 + 12) * .10;
    const slow = windNoise(clock * .29, i + 5.6) * .12;
    l.intensity = 2.6 * (1 + f * .3) * (.94 + flicker + slow - wind.gust * .025);
  });
  ...
  if (WORLD.haze) WORLD.haze.forEach(h => {
    const u = h.userData;
    h.position.x += dt * wind.x * u.sp;
    if (h.position.x > 36) h.position.x = -36;
    h.position.y = u.y0 + windNoise(clock * .1, u.ph) * .7;
    h.material.opacity = u.opacity * (.8 + windNoise(clock * .07, u.ph + 7) * .55);
    h.scale.x = 1.25 + windNoise(clock * .06, u.ph) * .25;
...
function renderFrame(now, animate = true) {
  if (disposed) return;
  ...
  if (animate && raw < .25 && !PERF.locked && clock > 2.2) {
    PERF.acc += raw; PERF.n++;
    if (PERF.n >= 40 || PERF.acc > .9) {
      const avg = PERF.acc / PERF.n; PERF.acc = 0; PERF.n = 0;
      if (avg > .024 && PERF.scale > .6) PERF.pending = Math.max(.6, PERF.scale - .2);
      else if (avg < .017 && clock - PERF.resizedAt > 8) PERF.pending = Math.min(1, PERF.scale + .2);
    }
    // Render-target allocation can stall the GPU. Defer it until the scroll
    // and camera have settled, with hysteresis instead of repeated resizing.
    if (PERF.pending !== PERF.scale && clock - RIG.scrolledAt > 1.2 && clock - PERF.resizedAt > 4) {
      PERF.scale = PERF.pending; PERF.resizedAt = clock; resize(false);
    }
  }

  RIG.smooth = animate ? damp(RIG.smooth, RIG.prog, 5.2, dt) : RIG.prog;
  RIG.intro = animate ? damp(RIG.intro, 1, .85, dt) : RIG.intro;
  updateWind();
  RIG.mx = damp(RIG.mx, RIG.tmx, 2.6, dt);
  RIG.my = damp(RIG.my, RIG.tmy, 2.6, dt);
  applyCamera();
  updateWorld(animate ? dt : 0);
  render();
}
...
return {
  ...
  setProgress(progress) {
    const next = clamp(progress, 0, CAM.length - 1);
    if (Math.abs(next - RIG.prog) > .0001) RIG.scrolledAt = clock;
    RIG.prog = next;
  },
